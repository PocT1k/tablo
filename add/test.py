from PyQt5.QtWidgets import (
    ...
    QDialog, QVBoxLayout, QInputDialog, QMessageBox
)
...
class SettingsWindow(QMainWindow):
    def __init__(self, parent):
        ...
        self.parent = parent
        # Настройки окна аналогично MainWindow, но меньше
        self.setWindowTitle("Настройки")
        self.setFixedSize(parent.win_w // 2, parent.win_h // 2)
        ...
        # Кнопка Информация
        info_btn = QPushButton("Информация", central)
        info_btn.clicked.connect(lambda: QMessageBox.information(
            self,
            "Информация",
            "Здесь вы можете вписать свою информацию о приложении..."
        ))
        layout.addWidget(info_btn)

        # Кнопка Переобучение распознавания лиц
        retrain_btn = QPushButton("Переобучение распознавания лиц", central)
        retrain_btn.clicked.connect(parent._retrain_model)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        archive_file(attendance_path, attendance_old_path)
    ...
    def _create_buttons(self):
        ...
        cam_display = cam_names[cam_idx] if cam_idx < len(cam_names) else (cam_names[0] if cam_names else "0: Камера 0")
        ...
        mic_names = [f"{i}: {d['name']}" for i, d in enumerate(inputs)]
        mic_idx = dict_get_or_set(self.setting_json, "microphone", 0)
        mic_display = mic_names[mic_idx] if mic_idx < len(mic_names) else (mic_names[0] if mic_names else "0: Микрофон 0")
        ...
        # Центральные две кнопки занимают оставшееся пространство
